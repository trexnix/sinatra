...
Sinatra là một [DSL](http://en.wikipedia.org/wiki/Domain-specific_language) dùng để 
nhanh chóng tạo ra các ứng dụng web trên Ruby với ít công sức nhất.
...
Xem tại: http://localhost:4567

Khuyến khích chạy luôn `gem install thin`, Sinatra sẽ dùng nó nếu có.
...
Ở Sinatra, một route là một *HTTP method* cặp với một *URL-matching pattern*.
Mỗi route được liên kết với một block:
...
(Ở các ví dụ trên, `get`, `post`, `patch`, `delete`, `link`, `unlink` là các HTTP method, và `'/'` là một URL-matching pattern)

Các route được khớp theo thứ tự mà chúng được định nghĩa. Route đầu tiên 
khớp với request sẽ được gọi.


Router patterns có thể chứa các tham số được đặt tên (*named parameters*), có thể truy cập được từ
hash `params`:

``` ruby
get '/hello/:name' do
  # khớp "GET /hello/foo" và "GET /hello/bar"
  # params['name'] là 'foo' hoặc 'bar'
  "Hello #{params['name']}!"
end
```

Bạn cũng có thể truy cập các tham số được đặt tên từ các tham số block (*block parameters*):

``` ruby
get '/hello/:name' do |n|
  # khớp "GET /hello/foo" và "GET /hello/bar"
  # params['name'] là 'foo' hoặc 'bar'
  # n lưu trữ params['name']
  "Hello #{n}!"
end
```

Các route pattern cũng có thể chứa các tham số *splat* (hay *wildcard*), có thể truy cập được
từ array `params['splat']`:

``` ruby
get '/say/*/to/*' do
  # khớp /say/hello/to/world
  params['splat'] # => ["hello", "world"]
end

get '/download/*.*' do
  # khớp /download/path/to/file.xml
  params['splat'] # => ["path/to/file", "xml"]
end
```

Hoặc với các tham số block:
...
Route khớp với các Regular Expression:
...
Hoặc với một tham số block:

``` ruby
get %r{/hello/([\w]+)} do |c|
  # Khớp "GET /meta/hello/world", "GET /hello/world/1234", v.v...
  "Hello, #{c}!"
end
```

Các route pattern cũng có thể có các tham số tùy chon:

``` ruby
get '/posts.?:format?' do
  # khớp "GET /posts" và bất kì phần mở rộng nào "GET /posts.json", "GET /posts.xml", v.v...
end
```

Các route cũng có thể tận dụng các tham số truy vấn (*query parameters*):

``` ruby
get '/posts' do
  # khớp "GET /posts?title=foo&author=bar"
  title = params['title']
  author = params['author']
  # sử dụng các biến title và author; các truy vấn thì không bắt buộc đối với route /post
end
```

(`?title=foo&author=bar` ở trên là các tham số truy vấn)
